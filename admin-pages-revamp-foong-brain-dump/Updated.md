...
Gaps flagged in earlier drafts and now FIXED in the backend, marked as such below:
questionnaire delete / add-option and assetType on the questionnaire lists (write-up 7), and
asset type department binding (write-up 8). The question ones need the pending
20260921090000_question_soft_delete migration deployed before they work in a given environment.

Correction to an earlier draft: there is no separate "retired" state. The column is isDeleted,
deleted questions and options are never returned by any endpoint, and there is no restore flow.
Do not build one.

Terminology: field names below are as the API returns them (camelCase, e.g. creditWeight) and
table names are as they exist in Postgres (snake_case, e.g. asset_entity). Where the source code
uses a name that is not a table - "rate card" is the only one - it is called out at the point of
use.
...
What it is
The entity is what gets invoiced under a client. baseCredits, the per-entity credit weights in
asset_entity, billingType, temp_credits, entity_notes and entity_files all live here. This is the
biggest admin surface of the eight.
...
Fields that need to be editable
  ...
  assetTypes[]        writes asset_entity rows, see below
...
Related records edited on the same page
  asset_entity        one row per asset type, each with a creditWeight and an optional name.
                      Drives what the client pays per unit of that asset type.
  multiplier_entities many-to-many with size_multiplier. Add/remove only, no edit.
  temp_credits        extra credits for a given month + year, with a TempCreditStatus
                      (pending / approved / rejected).
  entity_notes        notes with isStarred, isClientNote, and files in entity_notes_files.
  entity_files        entity-level file attachments, separate from note attachments.
...
- IMPORTANT: sending assetTypes[] on PUT /entity/:id is a FULL REPLACE. Any asset row you
  ...
  not a delta. asset_entity rows whose asset type has a departmentId are the one exception -
  they are excluded from the reconciliation and managed on the asset type instead.
- You cannot change assetTypeId on an existing asset_entity row. Remove it and add a new one.
- An asset type with a departmentId is limited to ONE live asset_entity row per entity. Adding
  a second is rejected with a 400.
- isValid is derived, not editable. It is true only when legalName is set AND baseCredits != 0
  AND invoice_fields.amount != 0. Read-only in the UI.
...
Tables touched
  ...
  asset_entity               per-entity credit weights (assetTypeId, creditWeight, name, isActive)
  asset_types                lookup, and the source of defaultCreditWeight
...
Fields that need to be editable
  ...
  departmentId            required on create. Decides which questions apply and which
                          asset_types row prices the work
...
Tables touched
  ...
  departments           lookup; drives questions and the department's asset_types row
  cities                lookup
  colors                lookup
  teams                 children
...
Related records edited on the same page
  project_team           a project can carry several rows, one isPrimary
  preferred_subtask_user a per-project, per-team preferred assignee, with a manualOverride flag
  project_notes          optionally scoped to a department, with isStarred
...
Question fields that need to be editable
  ...
  assetTypeId           quantity questions ONLY, and optional. Rejected with a 400 on any
                        other type. Returned on the questionnaire lists with the asset type's
                        id, name and capacityWeight so the editor can show the link
  isMultiplier          radio and checkbox ONLY. Rejected with a 400 on any other type
  isCustom
  workCapacityBase      the multiplier factor. Only read on a radio/checkbox question with
                        isMultiplier set - ignored everywhere else. Defaults to 1, so an
                        unconfigured multiplier does nothing rather than zeroing the subtask

Delete
  isDeleted             DELETE /questionAnswers/question/:questionId. Soft delete; the question
                        vanishes from every read. See rules
...
Per-type detail, each with its own edit endpoint
  ...
  question_options      radio and checkbox options - name, position, isCustom, isDeleted.
                        Options can now be added to an existing question and deleted individually.
...
- Deleting a question is a SOFT delete (isDeleted on the row), and the endpoint is a toggle so
  it can be undone by someone with API access. From the UI's point of view it is just gone:
    * a deleted question is never returned by the department questionnaire, the studio
      questionnaire or the subtask form, and can no longer be answered (400 if something tries);
  ...
  so dropping a deleted question from the maths would silently reduce open work's capacity and
  credits the next time anyone edited an unrelated answer. Deleting a question stops it collecting
...
- The two capacity mechanisms are now separate, and each is enforced on create AND edit:
    * assetTypeId  -> quantity questions only. Adds to capacity: capacityWeight x answered value.
    * isMultiplier -> radio/checkbox only. Multiplies capacity when ticked, by the question's
                      own workCapacityBase.
  A question never has both. Sending the wrong one is a 400, not a silent drop - it used to be
  silently nulled on create and accepted on edit, which is how multipliers ended up needing a
  two-step create-then-edit to work at all.
- A quantity question with no assetTypeId contributes NOTHING to derived capacity. That is the
  warning worth showing. The weight is assetType.capacityWeight, never workCapacityBase.
- A multiplier question with workCapacityBase of 1 (the default) is a no-op. Worth surfacing
  too, since it looks configured but changes nothing.
- review_details holds the per-department review checklist (name, type, description,
  goodExampleUrl, badExampleUrl, position, isDefault, isActive). Read-only via API today.
  Note the table is review_details; department_review_details is only the Prisma relation name.
...
- Deleted questions and options are NEVER returned by any questionnaire endpoint. There is no
  "show deleted" option and no restore flow in the UI - do not build one. isDeleted is on the
  response purely so the shape matches the table; it will always be false on anything you get
  back from a list.
...
What it is
asset_types carries the two numbers everything else derives from: capacityWeight (how much work
one unit is) and defaultCreditWeight (what one billed hour costs when the entity has no
asset_entity row of its own). size_multiplier is a separate, simpler scaling list.
...
Asset type fields that need to be editable
  ...
  departmentId           binds this asset type to a department. Nullable, at most one asset
                         type per department. Send null to unbind. See rules
...
- An asset type with a departmentId behaves differently from one without:
  it is limited to one live asset_entity row per entity, it is excluded from the entity edit
  screen's asset reconciliation, it is hidden from GET /assetTypes (so it never reaches the list
  used to add task_asset rows), and it is reached by derivation (subtask -> studio -> department)
  rather than being chosen by anyone. The code type for this is DepartmentRateCard, so you will
  see "rate card" in the source - there is no such table, it means asset_types + asset_entity.
- Setting departmentId is validated before the write, so you get a readable 400 rather than a
  database constraint error:
    ...
    * 'Department Rendering is already the rate card for asset type 7 "Render hour". Unbind that
      one first.' if something already holds it (wording as the API returns it).
...
- Creating an asset type with an entityId in the body also creates that entity's asset_entity
  row in the same transaction.
- Size multipliers attach to entities (multiplier_entities) and to individual tasks
  (multiplier_task). Changing a size_multiplier's value affects everything referencing it.
...
Tables touched
  asset_types                    capacityWeight and defaultCreditWeight
...
================================================================================
RESOLVED - HOW MULTIPLIER QUESTIONS WORK NOW
================================================================================

This replaces the open question in the previous draft. It is settled and built.

The rule
  assetTypeId   quantity questions only
  isMultiplier  radio and checkbox only
  A question never carries both.

A multiplier's factor is the question's own workCapacityBase, not an asset
type's capacityWeight. An asset type prices an answered quantity; a multiplier
has no quantity to price, it is ticked or it is not.

Why it changed
Between 2026-08-27 and 2026-09-04 the two mechanisms ended up sharing
questions.asset_type_id. createQuestion nulls that column on anything that is
not a quantity question, so a radio or checkbox multiplier could only be wired
up by creating it and then editing it, because editQuestion had no matching
guard. Both write paths now enforce the same rules and return a 400 instead of
silently dropping the field.

What FE needs to know
- isMultiplier is safe to ship now. It was not in the previous draft.
- Show workCapacityBase as the factor, on radio/checkbox multiplier questions
  only. Hide it everywhere else - it is ignored there.
- It defaults to 1. A multiplier left at 1 does nothing; worth a warning.
- A value of 0 or less is treated as unconfigured and ignored, and the capacity
  breakdown reports that line with applied: false so the reason is visible.

BREAKING response change
The subtask capacity breakdown's multiplierLines entries no longer carry
assetTypeId or assetTypeName. A multiplier has no asset type, so those fields
had no value to report. Remaining fields: questionId, questionName, multiplier,
applied. derivedLines are unchanged and still carry the asset type.

Needs the 20260921120000_multiplier_factor_off_asset_type migration deployed.
It copies each firing multiplier's capacityWeight onto its workCapacityBase, so
no open subtask is repriced.
