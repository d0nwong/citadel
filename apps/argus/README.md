...
| `scripts/marauder.ts` | `marauder board \| show \| changelog \| render` — the read verbs over `workstreams/`. Reads no network, no `.state/` and no clock beyond `--now`, so a run that changes no record writes no byte; a page that breaks a style rule is never written and the run exits non-zero naming the line. `accio` is untouched by it |
