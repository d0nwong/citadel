...
| `scripts/marauder.ts` | `marauder ingest \| board \| show \| changelog \| render` — the verbs over `workstreams/`. `ingest --landings` turns every merge on `origin/staging` and `origin/dev` since that side's newest landing into a `verified-landing` event and advances that side's stage; a merge two workstreams claim, or none, goes to `workstreams/_unsorted.json` and no record changes (LIA-156). Nothing here reads the network, no `.state/` and no clock beyond `--now`, so a run that changes no record writes no byte; a page that breaks a style rule is never written and the run exits non-zero naming the line. `accio` is untouched by it |
