...
- **Write nothing.** Not to this checkout, not to the product checkouts, not to Linear.
  ...
  a verdict and answers with it, and Liam's Confirm is what writes the file.
...
Never perform the action; section 5 is how a verdict reaches Liam.
...
The verdict on a point is Liam's; this section is only how it reaches him.

**When.** He asks for it — "ignore it", "send it to Foundry" — or he takes the
...
file lands when Liam presses Confirm, so never say the point is ignored, sent, decided or
...
Liam can still decide on the Points page.
