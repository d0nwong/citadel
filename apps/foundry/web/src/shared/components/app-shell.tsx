...
function Wordmark() {
  return (
    <span className="flex items-center gap-2.5">
      ...
      <span className="text-[15px] font-extrabold uppercase tracking-[0.2em] text-txt">
        Foundry
      </span>
    </span>
  );
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active
                  ? "bg-iron-800 text-txt"
                  : "text-txt-dim hover:bg-iron-850 hover:text-txt",
              )}
            >
              {active && (
                <span className="absolute left-0 top-1/2 h-4 w-0.5 -translate-y-1/2 rounded-r bg-ember" />
              )}
              <Icon
                className={cn(
                  "size-4",
                  active
                    ? "text-ember"
                    : "text-txt-faint group-hover:text-txt-dim",
                )}
              />
              {label}
            </Link>
    ...
    <div className="relative z-10 flex min-h-screen flex-col lg:flex-row">
      ...
      <header className="sticky top-0 z-40 flex items-center gap-3 border-b border-hairline bg-iron-950/85 px-4 py-3 backdrop-blur-md lg:hidden">
        <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
          ...
          <SheetContent
            side="left"
            className="flex w-64 flex-col gap-0 border-r border-hairline bg-iron-950 p-0"
          >
      ...
      <main className="grid-field flex min-h-0 flex-1 flex-col lg:ml-56">
        {children}
      </main>
    </div>
  );
