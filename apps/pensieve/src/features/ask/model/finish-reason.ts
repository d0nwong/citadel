...
export const finishReasonOf = (chunk: StreamChunk): string | undefined => {
  if (chunk.type !== 'RUN_FINISHED') return undefined
  const top = (chunk as { finishReason?: string | null }).finishReason
  const meta = (chunk as { metadata?: { tanstack?: { finishReason?: string | null } } }).metadata?.tanstack?.finishReason
  return top ?? meta ?? undefined
}

/** This session's finish for the thread; before any run here, what the stored conversation says. */
export const useFinishReason = (threadId: string | undefined, stored: string | undefined) =>
  useSyncExternalStore(
    subscribe,
    () => (threadId && finishReasons.has(threadId) ? finishReasons.get(threadId) : stored),
    () => stored,
  )
