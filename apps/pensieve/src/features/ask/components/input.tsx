  ...
  return (
    <PromptInput className="mt-4" onSubmit={submit}>
      <PromptInputTextarea
        ...
        placeholder={
          status.available
            ? "Ask about the argus checkout…"
            : "Ask needs a credential"
        }
        ref={field}
      />
      <PromptInputToolbar>
        ...
        {busy ? (
          ...
          <PromptInputSubmit
            disabled={!status.available}
            status={chat.status === "error" ? "error" : "ready"}
            title="Send"
          />
        )}
      </PromptInputToolbar>
      {!status.available && (
        <p className="border-border border-t px-4 py-2 text-sm text-st-hold leading-snug">
          {status.reason}. On this Mac,{" "}
          <code className="mono">claude login</code> — and if you are logged in
          already, a macOS Keychain dialog may be waiting for the{" "}
          <code className="mono">claude</code> process: choose Always Allow. In
          the container, set <code className="mono">ANTHROPIC_API_KEY</code>.
          Then reload.
