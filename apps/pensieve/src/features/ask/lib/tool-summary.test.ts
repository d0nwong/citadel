...
describe("toolSummary", () => {
  test("Read shows the path", () => {
    expect(toolSummary("Read", { file_path: "reports/points.json" })).toBe(
      "reports/points.json"
    );
  ...
  test("Grep and Glob show the pattern, and where when given", () => {
    ...
    expect(toolSummary("Grep", { path: "journal", pattern: "LIA-94" })).toBe(
      "LIA-94 in journal"
    );
    expect(toolSummary("Glob", { path: "docs", pattern: "**/*.md" })).toBe(
      "**/*.md in docs"
    );
  });
...
describe("parseArguments", () => {
  test("parses complete JSON, undefined otherwise", () => {
    expect(parseArguments('{"file_path":"a.md"}')).toEqual({
      file_path: "a.md",
    });
...
describe("toolResultText", () => {
  test("strings pass through, parts join their text, anything else is empty", () => {
    ...
    expect(
      toolResultText([
        { content: "a", type: "text" },
        { source: { type: "url", value: "x" }, type: "image" },
        { content: "b", type: "text" },
      ])
    ).toBe("ab");
