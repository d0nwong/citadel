...
export const WORKSPACE_DIR = resolve(process.env.WORKSPACE_DIR || join(homedir(), 'git/argus'))
...
const NOT_APPS = new Set(['reports', 'digests', 'skills', 'scripts', 'node_modules', 'features', 'dist'])
...
export async function listApps(): Promise<AppRoot[]> {
  ...
  const scan = async (rel: string, depth: number): Promise<void> => {
    ...
    for (const e of entries) {
      if (!e.isDirectory() || e.name.startsWith('.') || NOT_APPS.has(e.name)) continue
...
const rootOf = (abs: string, roots: AppRoot[]) => roots.find((r) => abs.startsWith(`${r.dir}/`))
 ...
 */
function splitKey(feature: string, roots: AppRoot[]): { root: AppRoot; dir: string } | undefined {
  for (const root of roots) {
    if (feature.startsWith(`${root.app}/`)) return { root, dir: feature.slice(root.app.length + 1) }
  }
  return undefined
}
 ...
 */
export async function render(absPath: string, feature?: string): Promise<Rendered> {
...
function journalMeta(abs: string, fm: Frontmatter, roots: AppRoot[]): JournalEntry | null {
  const root = rootOf(abs, roots)
  if (!root) return null
...
export async function readJournalEntry(id: string): Promise<(Rendered & { meta: JournalEntry }) | null> {
  ...
  outer: for (const root of roots) {
    for await (const p of walk(root.dir)) {
      ...
      if (feature === '' || key?.endsWith(`/${feature}`)) candidates.push(p)
    }
  }
...
function docMeta(abs: string, fm: Frontmatter, roots: AppRoot[]): DocMeta | null {
  const root = rootOf(abs, roots)
  if (!root) return null
 ...
 */
async function resolveDocPath(feature: string, tier: 'product' | 'arch', roots: AppRoot[]): Promise<string | null> {
