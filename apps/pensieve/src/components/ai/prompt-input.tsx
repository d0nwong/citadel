...
export function PromptInput({ className, onSubmit, ...props }: ComponentProps<'form'>) {
  return (
    <form
      className={cn('overflow-hidden rounded-2xl border border-border bg-card/80 shadow-sm backdrop-blur', className)}
      onSubmit={onSubmit}
      {...props}
    />
  )
...
export function PromptInputButton({ className, ...props }: ComponentProps<typeof Button>) {
  return <Button className={className} size="icon-sm" type="button" variant="ghost" {...props} />
}
...
export function PromptInputSubmit({
  ...
  return (
    <Button className={cn('rounded-full', className)} size="icon-sm" type="submit" {...props}>
      {icon}
