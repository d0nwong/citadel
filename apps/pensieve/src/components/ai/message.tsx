...
export function MessageAvatar({ name, src, className }: MessageAvatarProps) {
  ...
  return (
    <div
      className={cn(
        "flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-full bg-secondary text-muted-foreground text-xs",
        "group-[.is-user]:ml-auto",
        className
      )}
    >
      {src ? (
        // biome-ignore lint/correctness/useImageSize: the avatar box is sized by its wrapper
...
export const MessageResponse = memo(function MessageResponseInner({
  className,
  children,
}: {
  className?: string;
  ...
  return (
    <Md
      className={cn(
        "size-full max-w-none text-sm [&>*:first-child]:mt-0 [&>*:last-child]:mb-0",
        className
      )}
      doc={text}
    />
