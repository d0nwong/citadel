/**
 * The map's content: every box, where it sits and what it says, and every arrow between
 * them. Pure data — `graph.ts` turns it into React Flow nodes and edges — so a change to
 * the /scope skill is a change here and nowhere else.
 *
 * Two phases sit side by side: on the left what happens with the user in the /scope
 * conversation, on the right what runs after filing.
 */

export type Actor = "you" | "scope" | "argus" | "linear" | "foundry" | "sweep" | "data";

export const ACTORS: Record<Actor, { label: string; color: string }> = {
  you: { label: "You", color: "var(--you)" },
  scope: { label: "/scope skill", color: "var(--scope)" },
...
export type Item = {
  ...
  actor: Actor;
  title: string;
  /** a step's second line, a file's contents */
  sub?: string;
...
export const ITEMS: Item[] = [
  ...
  {
    id: "ground", kind: "step", x: 380, y: 120, actor: "scope", title: "1 · Ground", sub: "Find the features and read what exists, before the first question",
  ...
  {
    id: "interview", kind: "step", x: 380, y: 250, actor: "scope", title: "Interview", sub: "One question at a time, each with a guess",
  ...
  {
    id: "specs", kind: "step", x: 380, y: 615, actor: "scope", title: "3 · Specs", sub: "Copy each baseline, edit the copy",
  ...
  {
    id: "doubt", kind: "step", x: 10, y: 735, actor: "scope", title: "Doubt pass", sub: "A fresh subagent sees only the artifact and its contract",
  ...
  {
    id: "plan", kind: "step", x: 380, y: 855, actor: "scope", title: "4 · Plan", sub: "The difference, cut into tickets",
...
/** the box shown in the panel when the page opens */
export const FIRST = "specs";
