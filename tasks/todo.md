...
- [ ] **Checks.** `bun test apps/foundry/image`, `bun run --filter foundry-web typecheck`, `db:migrate`, `bun run --filter foundry-web test`.
- [ ] **T4: Prove on jobs.** AC1 (a bug ticket with steps → PR, first test red then green, root cause in the body), AC2 (a bug ticket with no steps → Blocked, no changes), AC3 (`/forge-debug` alone with a failing log → commit or a reason). Job ids in the PR body.
