 ...
 * router — ask once per provider, merge the answers, write nothing. `linearProvider` and
 * `trelloProvider`, each with its three standalone reads, are the adapters; `CTD` and `ALD`
 * route to Linear, `AP` to Trello.
 */

export { providerNameFor, providerNameForTeam, splitKey } from "./key.ts";
export { LINEAR_API_URL, linearGet, linearListOpen, linearProvider, linearTicketStates, stateOf } from "./linear.ts";
...
export { getTicket, listOpenTickets, ticketStates } from "./router.ts";
